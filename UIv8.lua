...
local DEFAULT_SECTION_ICON = "rbxassetid://7733960981" -- menu/list icon
...
function Library:New(config)
	...
	-- Used by both Tab (directly on page) and Section (inside section content)
	-- ============================================
	...
	function self:NewTab(name)
		...
		-- Add all standard element methods directly to Tab (for backward compatibility)
		...
		function Tab:Section(config)
			...
			-- Main section container (clips content when collapsed)
			...
			-- Description (optional, below title line)
			local descHeight = 0
			if sectionDesc then
			...
			-- Content holder (elements go here)
			local ContentHolder = Create("Frame", {
				Name = "SectionContent",
				Size = UDim2.new(1, -16, 0, 0), -- Auto-sized
			...
			-- Add all element methods to Section
